...
function jsonCallback(json){
	...
	var success =  true;
	if( json.hasOwnProperty("Banner_728x90") ) {
		content = json.Banner_728x90;
	} else {
		success = false;
		alert("ERROR: Please check that the data file has a 'Banner_728x90' node. Refer to the documentation for guidance.");
	}
	if( content.hasOwnProperty("loop") ) {
		loop = content.loop;
	} else {
		success = false;
		alert("ERROR: Please check that the data file object 'Banner_728x90' has a 'loop' node. Refer to the documentation for guidance.");
	}
	if( success ) {
		preloadImages();
	}
...
function preloadImages() {
	...
	queue.loadManifest(getPreloadImagesManifest(content, data.Assets_728x90, data));
	queue.loadManifest(getPreloadRoundelImagesManifest(data.Assets_Roundel, data.Roundel_ID, "728x90"));
}
