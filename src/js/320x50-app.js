...
function jsonCallback(json){
	...
	var success =  true;
	if( json.hasOwnProperty("Banner_320x50") ) {
		content = json.Banner_320x50;
	} else {
		success = false;
		alert("ERROR: Please check that the data file has a 'Banner_320x50' node. Refer to the documentation for guidance.");
	}
	if( content.hasOwnProperty("loop") ) {
		loop = content.loop;
	} else {
		success = false;
		alert("ERROR: Please check that the data file object 'Banner_320x50' has a 'loop' node. Refer to the documentation for guidance.");
	}
	if( content.hasOwnProperty("legal-label-position") ) {
		legalLabelPosition = content["legal-label-position"];
	} else {
		success = false;
		alert("ERROR: Please check that the data file object 'Banner_320x50' has a 'legal-label-position' node. Refer to the documentation for guidance.");
	}
	if( success ) {
		preloadImages();
	}
...
function preloadImages() {
	...
	queue.loadManifest(getPreloadImagesManifest(content, data.Assets_320x50, data));
	queue.loadManifest(getPreloadRoundelImagesManifest(data.Assets_Roundel, data.Roundel_ID, "320x50"));
}
