...
function checkBrowserIfSafari(){
  ...
  if(Object.prototype.toString.call(window.HTMLElement).indexOf('Constructor') > 0){
    ...
    elm.setAttribute("href","css/120x600_safari.css");
...
function jsonCallback(json){
	...
	var success =  true;
	if( json.hasOwnProperty("Banner_120x600") ) {
		content = json.Banner_120x600;
	} else {
		success = false;
		alert("ERROR: Please check that the data file has a 'Banner_120x600' node. Refer to the documentation for guidance.");
	}
	if( content.hasOwnProperty("loop") ) {
		loop = content.loop;
	} else {
		success = false;
		alert("ERROR: Please check that the data file object 'Banner_120x600' has a 'loop' node. Refer to the documentation for guidance.");
	}
	if( success ) {
		preloadImages();
	}
...
function preloadImages() {
	...
	queue.loadManifest(getPreloadImagesManifest(content, data.Assets_120x600, data));
	queue.loadManifest(getPreloadRoundelImagesManifest(data.Assets_Roundel, data.Roundel_ID, "120x600"));
}
...
function animateIn(object, inAnimation, outAnimation, delayBetweenAnimations) {
	switch (inAnimation.type) {
		...
		case "slide" :
			if ( inAnimation.hasOwnProperty("origin") ) {
				if ( inAnimation.origin === "right" ) {
					...
					object.regY = Math.floor(object.image.height * 0.5);
					object.y = 262;
					object.x = 120 + Math.floor(object.image.width * 0.5);
					object.alpha = 0;
					stageContent.addChild(object);
					createjs.Tween.get(object).to({x:59,alpha:1},parseInt(inAnimation.duration),createjs.Ease.sineOut).wait(parseInt(delayBetweenAnimations)).call(function() {
						animateOut(object, outAnimation);
					});
		...
		case "cut" :
			...
			stageContent.addChild(object);
			/*
			if(stageContent.getChildByName('image-1')) {
				stageContent.removeChild(stageContent.getChildByName('image-1'));
			}*/
...
function animateOut(object, outAnimation) {
	...
	switch (outAnimation.type) {
		...
		case "slide" :
			if ( outAnimation.hasOwnProperty("destination") ) {
				...
				if ( outAnimation.destination === "left" ) {
					...
					finalOffset = 120 + Math.floor(object.image.width * 0.5);
					createjs.Tween.get(object).to({x:finalOffset,alpha:0},parseInt(outAnimation.duration),createjs.Ease.sineOut).call(function() {
						stageContent.removeChild(object);
					});
		...
		case "cut" :
			console.log("works");
			// createjs.Tween.get(object,{override:true}).wait(outAnimation.duration).call(function() {
			// 	//stageContent.removeChild(object);
			break;			
	}
}
